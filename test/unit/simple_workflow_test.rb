...
class SimpleWorkflowTest < ActiveSupport::TestCase
  ...
  test 'should be current when newly created' do
    assert_equal :current, create(:policy_area).current_state
  end
  ...
  test "should not call rummager if Searchable is not included" do
    topic = create(:policy_area)
    PolicyArea.any_instance.stubs(:remove_from_search_index).returns(NameError)
  ...
  test "should exclude deleted topics by default" do
    current_topic = create(:policy_area)
    deleted_topic = create(:policy_area, state: "deleted")
    assert_equal [current_topic], PolicyArea.all
  end
end
